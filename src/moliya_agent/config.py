    ...
    def from_env(cls) -> Settings:
        ...
        settings = cls(
            ...
            session_cookie_secure=os.getenv(
                "MOLIYA_SESSION_COOKIE_SECURE", ""
            ).strip().lower() in {"1", "true", "yes"},
            web_dist_dir=(
                Path(value)
                if (value := os.getenv("MOLIYA_WEB_DIST_DIR", "").strip())
                else None
            ),
        )
        settings.validate()
    ...
    def validate(self) -> None:
        if self.parser_mode not in {"rule", "openai", "hermes"}:
            raise ConfigurationError(
                "MOLIYA_PARSER_MODE rule, openai yoki hermes bo'lishi kerak"
            )
